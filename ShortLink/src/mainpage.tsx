...
// Define the structure of the short link object
interface ShortLink {
  shortLink: string;
  title: string;
  originalLink: string;
...
function App() {
  ...
  // Function to handle click on the short link (increases the click count)
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (originalUrl && title) {
      setShowProtectModal(true); // Show modal to configure link options
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center p-6">
      ...
      <div className="mt-8 w-full max-w-4xl">
        ...
        <form onSubmit={handleSubmit} className="flex items-center bg-white p-4 rounded-lg shadow-md mb-6">
          ...
          <input
            type="text"
            className="ml-4 flex-grow p-2 border rounded-lg outline-none"
            placeholder="Enter title"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
          />
            ...
            {shortlinks.length > 0 ? (
              shortlinks.map((link, idx) => (
                <tr key={idx} className="border-t">
                  ...
                  <td className="p-4">{link.date}</td>
                  <td className="p-4 flex space-x-2">
                    <button onClick={() => handleEdit(link)} className="text-blue-600 underline">
                      Edit
                    </button>
                  </td>
                </tr>
      ...
      {/* Show Modal if user clicked "Shorten Now" or "Edit" */}
