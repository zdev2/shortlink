...
const LandingPage = () => {
  ...
  const handleLogin = async () => {
    try {
      const response = await fetch(
        "https://shortlink-production-dnds.up.railway.app/api/v1/users/login",
        {
          method: "POST",
          headers: {
  ...
  const handleRegister = async () => {
    try {
      const response = await fetch(
        "https://shortlink-production-dnds.up.railway.app/api/v1/users/register",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className=" min-h-screen text-gray-900  md:pt-14">
      ...
      {isLoginPopupVisible && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 popup-background show">
          <div className="bg-white rounded-lg p-4 shadow-lg w-[250px] popup-container show md:w-[350px] md:px-10 lg:py-10 ">
            <button onClick={closeLoginPopup} className="font-bold text-2xl">
              <img src={close} alt="" className="w-5" />
            </button>
            <h2 className="text-2xl font-bold mb-4 text-center lg:text-4xl">
              Welcome Back
            </h2>
      ...
      {isRegisterPopupVisible && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 popup-background show">
          <div className="bg-white rounded-lg p-4 shadow-lg w-[250px] popup-container show  md:w-[350px] md:px-10 lg:py-10 ">
            <button onClick={closeRegisterPopup} className="">
              <img src={close} alt="" className="w-5" />
            </button>
            <h2 className="text-2xl font-bold mb-4 text-center lg:text-4xl">
              Get's Started
